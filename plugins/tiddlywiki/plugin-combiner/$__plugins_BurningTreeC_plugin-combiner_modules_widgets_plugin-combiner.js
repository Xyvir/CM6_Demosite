/*\
title: $:/plugins/BurningTreeC/plugin-combiner/modules/widgets/plugin-combiner.js
...
PluginCombinerWidget.prototype.render = function(parent, nextSibling) {
    ...
    this.includeDepsCheckbox.checked = true; // On by default
    ...
    var configSection = this.document.createElement("details");
    configSection.className = "tc-plugin-combiner-config";

    var summary = this.document.createElement("summary");
    summary.textContent = "Output plugin settings";
    configSection.appendChild(summary);

    var configFields = this.document.createElement("div");
    configFields.className = "tc-plugin-combiner-fields";

    this.outputFields = {};

    var fields = [
        {name: "title", label: "Plugin title", default: "$:/plugins/custom/combined"},
        {name: "name", label: "Display name", default: "Combined Plugin"},
        {name: "version", label: "Version", default: "1.0.0"},
        {name: "author", label: "Author", default: ""},
        {name: "description", label: "Description", default: "A combined plugin bundle"}
    ];

    fields.forEach(function(field) {
        var row = self.document.createElement("div");
        row.className = "tc-plugin-combiner-field";
        ...
        self.outputFields[field.name] = input;
    ...
    var installBtn = this.document.createElement("button");
    installBtn.className = "tc-btn-big-green";
    installBtn.textContent = "Install to Wiki";
    installBtn.style.marginLeft = "0.5em";
...
PluginCombinerWidget.prototype.buildCombinedPlugin = function() {
    ...
    // Build output plugin
    var outputTitle = this.outputFields.title.value.trim();
    if (!outputTitle) {
    ...
    var combinedPlugin = {
        title: outputTitle,
        name: this.outputFields.name.value || "Combined Plugin",
        description: this.outputFields.description.value || ("Combined from: " + sourcePlugins.join(", ")),
        author: this.outputFields.author.value || "",
        version: this.outputFields.version.value || "1.0.0",
        "plugin-type": "plugin",
        type: "application/json",
        text: JSON.stringify({tiddlers: combinedTiddlers})
    };
...
PluginCombinerWidget.prototype.handleInstall = function() {
    try {
        var result = this.buildCombinedPlugin();
        var plugin = result.plugin;

        // Add to wiki
        this.wiki.addTiddler(new $tw.Tiddler(plugin));

        var message = "Installed \"" + plugin.name + "\" with " + 
            result.tiddlerCount + " tiddlers";
        if (result.addedDependencies > 0) {
            message += " (" + result.addedDependencies + " dependencies auto-included)";
        }
        message += ". Reload to activate.";
        this.showStatus("success", message);
    } catch (e) {
        this.showStatus("error", e.message);
    }
